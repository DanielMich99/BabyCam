                    ...
                    for xyxy, conf, cls in zip(results.boxes.xyxy, results.boxes.conf, results.boxes.cls):
                        if conf > 0.1:
                            ...
                            if not last_time or (now - last_time).total_seconds() > 5:
                                ...
                                relative_path = save_detection_image(base_path, profile_id, camera_type, class_name, class_obj.id, xyxy, frame)
...
def save_detection_image(base_path, baby_profile_id, camera_type, class_name, class_id, xyxy_tensor, frame):
