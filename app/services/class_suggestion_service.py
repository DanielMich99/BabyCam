...
# Uses AI to suggest object detection class names based on baby profile data and camera type
def suggest_classes_for_baby_profile(
    db: Session, user_id: int, baby_profile_id: int, camera_type: str
) -> List[str]:
    """
    Fetches baby profile, generates a detailed prompt, sends it to ChatGPT, and returns suggested class names.
    """

    # Validate ownership of the baby profile
    ...
    # Compose prompt for AI to generate hazard-related class suggestions
    prompt = (
    ...
    # Get response from AI model and return list of stripped class names
