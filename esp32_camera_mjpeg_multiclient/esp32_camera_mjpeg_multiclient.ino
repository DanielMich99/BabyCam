...
// 🆕 כתובת השרת שלך (שמתארח עליו FastAPI)
const char* backend_url_gil = "http://192.168.1.63:8000/camera/report_ip";  // שנה ל-IP של השרת שלך
// 🆕 כתובת השרת שלך (שמתארח עליו FastAPI)
const char* backend_url_gil_home = "http://10.100.102.8:8000/camera/report_ip";  // שנה ל-IP של השרת שלך
// 🆕 כתובת השרת שלך (שמתארח עליו FastAPI)
const char* backend_url_daniel = "http://192.168.1.228:8000/camera/report_ip";  // שנה ל-IP של השרת שלך
...
// 🆕 הוספת /identify
void handleIdentify() {
  server.send(200, "application/json", "{\"device\": \"ESP32-CAM\", \"project\": \"babycam\"}");
}

// 🆕 פונקציה ששולחת את ה-IP לשרת
void reportIPToServer(String ip, String url) {
...
void setup()
{
  ...
  reportIPToServer(WiFi.localIP().toString(), backend_url_gil);
  reportIPToServer(WiFi.localIP().toString(), backend_url_gil_home);
  reportIPToServer(WiFi.localIP().toString(), backend_url_daniel);

  // 🆕 הוספת הנתיב /identify לשרת
  server.on("/identify", handleIdentify);  // מזהה את עצמו
...
void loop() {
  ...
  Serial.println(WiFi.RSSI());  // מדפיס את עוצמת האות
  delay(5000);  // כל 5 שניות
}
